...
class RedisMetastoreClient(MetastoreClientBase):
    ...
    def __init__(
        ...
        self.host = _get_config_value(
            lm_service_envs.LM_SERVICE_REDIS_IP, "metastore_host"
        )
        self.port = _get_config_value(
            lm_service_envs.LM_SERVICE_REDIS_PORT, "metastore_port"
        )
        self.db = _get_config_value(
            lm_service_envs.LM_SERVICE_REDIS_DB, "metastore_db"
        )
        self.password = _get_config_value(
            lm_service_envs.LM_SERVICE_REDIS_PASSWORD, "metastore_password"
        )
        self.url = _get_config_value(
            lm_service_envs.LM_SERVICE_REDIS_URL, "metastore_url"
        )
    ...
    def _initialize_clients(self):
        ...
        try:
            if self.url is not None:
                ...
                logger.info(f"Redis client initialized with url={self.url}")
            else:
                ...
                logger.info(
                    f"Redis client initialized with host={self.host}, port={self.port}"
                )
