...
class DisaggWorker:
    ...
    async def _send_autodiscovery_request(
        ...
        for socket in socket_list:
            ...
            logger.info(
                f"Worker {self.worker_addr} sent autodiscovery request to proxy"
            )
    ...
    async def run_busy_loop(self):
        ...
        if self.metastore_client is None:
            discovery_task = asyncio.create_task(
                self._send_autodiscovery_request(self.to_proxy.values())
            )
