...
DECODER_ADDR_PREFIX="${DECODE_ADDR_PREFIX:-/tmp/decoder}"
...
function start_decoder() {
    ...
    VLLM_USE_V1=1 ASCEND_RT_VISIBLE_DEVICES=$dev_id python -m llm_service.entrypoints.worker \
        ...
        --worker-addr $address \
        --model $MODEL \
        --gpu-memory-utilization $GPU_UTILIZATION_DECODER \
...
chat_with_image() {
    python $CURRENT_DIR/chat_with_image.py \
        ...
        --prefill-addr-list $(for ((i=0; i<PD_NUMBER; i++)); do echo -n "${P_ADDR_PREFIX}_$i "; done) \
        --decode-addr-list $(for ((i=0; i<PD_NUMBER; i++)); do echo -n "${D_ADDR_PREFIX}_$i "; done) \
        --model-name $MODEL \
