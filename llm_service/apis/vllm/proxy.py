...
class Proxy(EngineClient):
    ...
    def __init__(
        ...
        if llm_service_envs.AUTO_DISCOVERY_SERVICE:
            ...
            self.proxy_addr = (
                f"{self.transfer_protocol}://{local_ip}:{proxy_port}"
            )
            if (
                is_addr_ipv6(self.proxy_addr)
        ...
        self.is_pd_merged = llm_service_envs.DEPLOY_FORM == "E-PD"
    ...
    def _initialize_instance_clusters(
        ...
        instance_router = (
            ROUTER_MAP.get(llm_service_envs.__getattr__(route_policy), None)
            or self.router
        )()
        return (
            service_discovery,
    ...
    async def get_metrics(self, server_type: ServerType, addr: str):
        ...
        try:
            ...
            if (
                ...
                if self.is_pd_model(server_type):
                    proxy2pd_avg = (
